...
function applyTimeFormat(format) {
  ...
  // Update selected button
  document.querySelectorAll(".theme-option[data-format-choice]").forEach((button) => {
...
// Format buttons
const formatButtons =
  document.querySelectorAll(".theme-option[data-format-choice]");
...
settingsNavButtons.forEach((navButton) => {
  ...
  navButton.addEventListener("click", () => {
    ...
    const targetPanel = navButton.dataset.panel;
    ...
    document.querySelectorAll(".settings-pane").forEach((pane) => {
      const isActive = pane.id === `pane-${targetPanel}`;
...
function renderShortcuts() {
  ...
  shortcuts.forEach((shortcut, index) => {
    ...
    iconButton.className = "shortcut-icon";
    iconButton.setAttribute("aria-label", `Open ${shortcut.name}`);
    ...
    if (icon) {
      ...
      img.onerror = () => {
        img.remove();
        iconButton.textContent = initials(shortcut.name);
      };
    ...
    removeButton.addEventListener("click", (event) => {
      ...
      const confirmed = window.confirm(`Remove "${shortcut.name}"?`);
      if (!confirmed) return;
  ...
  addButton.className = "shortcut-icon";
...
function openShortcutDialog() {
  ...
  setTimeout(() => shortcutNameInput.focus(), 50);
}
...
saveShortcutButton.addEventListener("click", addShortcut);
cancelShortcutButton.addEventListener("click", closeShortcutDialog);
shortcutBackdrop.addEventListener("click", closeShortcutDialog);
...
document.addEventListener("keydown", (event) => {
  if (event.key === "Escape" && shortcutDialog.classList.contains("open")) {
    closeShortcutDialog();
  }
});
